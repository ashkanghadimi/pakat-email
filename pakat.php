<?php
/**
 * Plugin Name: Pakat Email Subscribe Form And WP SMTP
 * Plugin URI: https://pakat.net/?r=wporg
 * Description: Easily send emails from your WordPress blog using Pakat SMTP and easily add a subscribe form to your site
 * Version: 2.9.3
 * Author: Pakat
 * Author URI: https://pakat.net/?r=wporg
...
if ( ! class_exists( 'SIB_Manager' ) ) {
	...
	class SIB_Manager {
		...
		/**
		 * To send the transactional email via Pakat
		 * hook wp_mail
		 *
		 * @param string $to - reception email.
		 * @param string $subject - subject of email.
		...
		static function language_admin_notice() {
			if ( ! get_option( SIB_Manager::LANGUAGE_OPTION_NAME ) ) {
				...
				$class = 'error';
				$message = sprintf( 'Please note that your Pakat account is in %s, but Pakat WordPress plugin is only available in English / French for now. Sorry for inconvenience.', $lang );
		...
		static function wpMailNotices() {
			if ( self::$wp_mail_conflict ) {
				echo ( '<div class="error"><p>' . __( 'You cannot use Pakat SMTP now because wp_mail has been declared by another process or plugin. ', 'sib_lang' ) . '</p></div>' );
			}
		}
